		...
		if (args.length == 0 || args[0] == null || args[0].trim().length() == 0) {
			...
			if (args.length > 0) {
				if (args.length == 1 && cmdVersion.equals(args[0])) {
					...
					String[] channel = argsMap.containsKey(cmdContentText) ? argsMap.get(cmdContentText).split(";") : null;
					...
					if (path != null) {
						...
						if (channel != null && channel.length > 0) {
							System.out.println("channel: " + Arrays.toString(channel));
						}
